...
def detect_face(image, net, min_confidence, prev, dims = (224, 224),flag=0,bbox_exp = 0.1, fill = 'reflect'):
    ...
    for face in results[0][0]:
        ...
        if (bbox > 1).any() or (bbox<0).any():
            continue
        if (face_confidence > min_confidence) and (area > 50*50) and (is_face > 0):
#             import pdb; pdb.set_trace()
            pad = 500
            ...
            if flag:
                ...
                if dims is None:
                    output_image = np.pad(image,((pad,pad), (pad,pad), (0,0)), mode=fill)[y1:y2,x1:x2]
                else:
...
#     e_map[:,:,2] = (e_map[:,:,2] - e_map[:,:,2].min()) / e_map[:,:,2].max()
    freqs, ESD = signal.welch(X, fs=fs, axis = 0, scaling='spectrum', nperseg = 24)
    ESD = ESD.sum(axis=0)
